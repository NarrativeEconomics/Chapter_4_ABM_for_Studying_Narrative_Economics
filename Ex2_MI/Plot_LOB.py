...
file_name = "bse_MI_min150_i05_0001_LOB_frames.csv"
...
book_top = 3
# buyer quote lines
for r in range(1, book_top):
    plt.plot(quotes_1s["second"], quotes_1s[f"bid_{r}"],
             drawstyle="steps-post", alpha=0.85, label = "bid")

# seller quote lines
for r in range(1, book_top):
    plt.plot(quotes_1s["second"], quotes_1s[f"ask_{r}"],
             drawstyle="steps-post", alpha=0.85, label = "ask")
